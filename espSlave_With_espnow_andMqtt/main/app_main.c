...
// global variables:
//------------------
  TaskHandle_t xTask_Mqtt;
  TaskHandle_t xTask_espnow;
...
// char *dataptr = "{JSON:test msg,data:{Testing thermal sensor payload and pixel data, reading data and writting data, 100,101,102,103,104,105,106,107,108,109,110,111,1}}:{JSON:test msg,data:{Testing thermal sensor payload and pixel data, reading data and writting data, 100,101,102,103,104,105,106,107,108,109,110,111,1}}:{JSON:test msg,data:{Testing thermal sensor payload and pixel data, reading data and writting data, 100,101,102,103,104,105,106,107,108,109,110,111,1}}:{JSON:test msg,data:{Testing thermal sensor payload and pixel data, reading data and writting data, 100,101,102,103,104,105,106,107,108,109,110,111,1}}: %d";
//char dataptr[] = "MQTT test";
...
void app_main(void)
{
    ...
    HWInit(); 
    xTaskCreate(&espnow_task, "espnow_task",2048, NULL, 4, &xTask_espnow);
    xTaskCreate(&mqtt_Task, "mqtt_Task", 5120, NULL, 12, &xTask_Mqtt);	
}
