...
{
    { 0x3C, 0x71, 0xBF, 0x4d, 0x4e, 0x78 } ///esp master MAC address
...
///response msg callback func
 jsonFunc cmdSendFunc[] = {
	 prepare_bootJSON,
	 prepare_sensorACKJSON,
     prepare_errorJSON,
	 NULL
 };

 volatile uint8_t ackSend = 0;
...
static void espnow_Rx_callback(const uint8_t *mac_addr, const uint8_t *data, int len)
{
    ...
    if (mac_addr == NULL || data == NULL || len <= 3) {
 ...
 * 
 * Supported msg:
 *   bootmsg: helloe >>>>> response given as "welcome"
 *   sensor msg: type:"sensor".... >>>>response given as "ok"
 *   invalid msg: >>>>resonse given as "error"
 *  
 * @param : none
 * 
 */
void espnow_StateMachine(void)
{
    ...
    switch(espnowState)
    {
        ...
        case ESPNOW_SEND_MSG:
        ///prepare response mesg
         cmdSendFunc[cmdSendFuncIndex](sendData, &sendDataLen, (int)SEND_DATA_MAX); 
        ...
        espnowState = ESPNOW_SEND_WAIT;
        break;  

        case ESPNOW_SEND_WAIT:
        ESP_LOGI(TAG, "state wait for send msg completed");
        if(xQueueReceive(espnow_queue_msg, &evt, 3000) == pdTRUE)
        {
          ...
          // memcpy(destMAC[DEST_MAC_SLAVE1], evt->smac_addr, ESP_NOW_ETH_ALEN);
            ESP_LOGD(TAG, "Send completed");
            espnowState = ESPNOW_RECV;
        ...
        {
           ESP_LOGI(TAG, "Recvd Data Len: %d,\n\r%s \n\r",evt.dlen, evt.data); 
               if(parse_bootJSON(evt.data, evt.dlen, 0) )//if valid string found 
               {
                    
                    ESP_LOGI(TAG, "valid boot msg found");
                    espnowState = ESPNOW_SEND_MSG;
                    cmdSendFuncIndex = CMD_SEND_BOOT_MSG;
               } 
               else if( parse_sensordataJSON(evt.data, evt.dlen, 0) )
               {
                    ESP_LOGI(TAG, "valid sensor msg found");
                    espnowState = ESPNOW_SEND_MSG;
                    cmdSendFuncIndex = CMD_SEND_SENSOR_MSG;
               }
               else
               {
                    ESP_LOGI(TAG, "invalid  msg found");
                    espnowState = ESPNOW_SEND_MSG;
                    cmdSendFuncIndex = CMD_SEND_ERROR_MSG;
               }
        }
        break;  
...
/**
 * @brief : check for espnow is initialize correctly or not
 * @param : none
 * 
 */
int isESPNOW_NotReady()
{
    if( espnowState == ESPNOW_FAIL )
    {
        return 1;
    }
